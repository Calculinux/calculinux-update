...
def get_overlay_dirs_for_path(
    file_path: Path, *, overlay_root: str = "/data/overlay"
) -> tuple[Optional[Path], Optional[Path]]:
    """Get overlay upper/lower file paths for a given absolute path.

    Calculinux mounts overlays for /etc, /root, /home, /var, /usr, /opt using:
      /data/overlay/<mountpoint>/upper/<relative-path>
      /data/overlay/<mountpoint>/lower/<relative-path>

    Example:
      /etc/foo.conf -> /data/overlay/etc/upper/foo.conf and /data/overlay/etc/lower/foo.conf
    """
    ...
    for overlay_mount in overlaid_paths:
        if path_str == overlay_mount or path_str.startswith(overlay_mount + "/"):
            ...
            base = Path(overlay_root) / overlay_mount.lstrip("/")
...
def detect_modified_conffiles(
    ...
    for conffile in image_conffiles:
        ...
        upper_file, lower_file = get_overlay_dirs_for_path(file_path, overlay_root=overlay_mount)
...
def create_dpkg_new_files(
    ...
    for conffile in modified_conffiles:
        ...
        _, lower_file = get_overlay_dirs_for_path(file_path, overlay_root=overlay_mount)
